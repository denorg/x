...
export default class JsonCompiler {
  ...
  protected re_ignore_block = new RegExp(/@ignore/, "g");
  protected re_ignore_line = new RegExp(/@ignore-line/);
  ...
  protected re_is_const = new RegExp(/export? ?const +\w+(: +\w+)? += +?.+/, "g");
  ...
  /**
   * @description
   *     A property to hold the name of the current file being parsed.
   *
  ...
  public compile(files: string[]): any {
    // This fixes the caching issue that happens when this object is reused
    this.parsed = {};
  ...
  protected parseMembersOnlyFile(fileContents) {
    ...
    docBlocks.forEach(docBlock => {
      if (this.re_ignore_block.test(docBlock)) {
        return;
      }
